...
template <typename T>
void COO<T>::gpu_compute(GPU_Pointers *pointers, uint grid_size,
                         uint blk_size) {
...
typename COO<T>::GPU_Pointers COO<T>::gpu_prep(const T *dense_vec) const {
    ...
    cudaMemcpy(pointers.matrix.row_p, this->matrix.row_p,
               nnz * sizeof(uint32_t), cudaMemcpyHostToDevice);
    cudaMemcpy(pointers.matrix.col_p, this->matrix.col_p,
               nnz * sizeof(uint32_t), cudaMemcpyHostToDevice);
...
template __global__ void spmv_coo_kernel<int>(COO_Matrix<int>, const int *,
                                              int *);
template __global__ void spmv_coo_kernel<float>(COO_Matrix<float>,
                                                const float *, float *);
template __global__ void spmv_coo_kernel<double>(COO_Matrix<double>,
                                                 const double *, double *);
